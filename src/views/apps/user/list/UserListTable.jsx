...
const UserListTable = ({ tableData }) => {
  ...
  const handleEdit = (id) => {
    setIsEditClick(true);
    const userIdMatch = data.userData.filter(item => item.id === id);

    setUserEditData(userIdMatch[0]);

    console.log('selected edit data', userIdMatch[0]);
    setAddUserOpen(true);
  };
  ...
  const handleSubmissionSuccess = (condition) => {
    debugger;
    setSubmitSuccess(condition);
  };
  ...
  return (
    <>
      <Snackbar open={submitSuccess} autoHideDuration={6000} onClose={handleCloseSnack} action={action}>
        <Alert
